...
//Constlar
const char AIDAT_ADRESI[] = "/home/enes/Masaüstü/aidat.txt";
const char TOPLAM_ADRESI[] = "/home/enes/Masaüstü/toplam.txt";
const char ISLEM_SAYISI = 15;
//Hepsi Farklıysa Önlem Olması için doğru yazın.
//Proje 15 sınırı olduğundan çok önem verilmemiştir
...
void ekle(struct daire* kayitlar,int* kullanici_sayisi) {
    ...
    struct daire gecici_kayit; //Struct Daire türünde geçici veri depolar olustur
    ...
    while (fgets(satir_verisi, sizeof(satir_verisi), dosya) != NULL) {
        ...
        // Proje ödevi dahilinde 'ISLEM_SAYISI'[Bize verilen görevde 15 kişi] istek sınırlandırılması için yapılmıştır.
        for (int i = 0; i < ISLEM_SAYISI+1; i++) {
            // Yine proje ödevi yüzünden max 'ISLEM_SAYISI' kişi olabilceğinden Index kişi sayısıyla(Kişi sayisi - 1) aynı olamacağından 'ISLEM_SAYISI'da otomatik kişi eklemesi için yapılmıştır.
            if (i == ISLEM_SAYISI) {
...
void kesme(struct daire *isleme, char veri[128]) {
    ...
    // tokenIndex: en son kaçıncı tokeni aldığımızı anlamamızı sağlar
    int tokenIndex = 0;
...
void yazdir(struct daire bilgiler[], int kullanici_sayisi) {
    ...
    //Her bir satırı tek tek yazma işlemi.
